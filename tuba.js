...
var fuelFieldset = document.getElementsByTagName("fieldset")[3];
    ...
    try {
        if(!(acresBox.value > 0)) {
            throw "Please enter a number of acres that is greater than 0.";
        }
} catch (message) {
        validity = false;
        messageText = message;
    ...
    try {
        for (var i = 0; i < 7; i++) {
            if (cropsFieldset.getElementsByTagName("input") [i].checked) {
                cropsComplete = true; messageElement.innerHTML = ""; // clear previous message or recommendations
        ...
        cropsComplete = false; messageHeadElement.innerHTML = ""; //remove any former recommendation headings
